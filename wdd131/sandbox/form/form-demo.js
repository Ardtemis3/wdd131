...
function validateForm(event) {
  ...
  function togglePaymentDetails(e) {
    ...
    const paypalContainer = document.querySelector("#paypalUserContainer");
    ...
    theForm.elements.cardNumber.required = false;
    theForm.elements.paypal.required = false;
    
    const theSelect = e.target;
    // Show the container based on the selected payment method, and add the required attribute back.
    if (theSelect.value == "credit"){
      creditCardContainer.classList.remove("hide");
      theForm.elements.cardNumber.required = true;
        } else if (theSelect.value == "paypal"){
      paypalContainer.classList.remove("hide");
      theForm.elements.paypal.required = true;
        }
  }
  ...
  document.querySelector("#paymentType").addEventListener("change", togglePaymentDetails);
